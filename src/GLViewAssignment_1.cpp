...
GLViewAssignment_1* GLViewAssignment_1::New( const std::vector< std::string >& args )
{
   GLViewAssignment_1* glv = new GLViewAssignment_1( args );
...
GLViewAssignment_1::GLViewAssignment_1( const std::vector< std::string >& args ) : GLView( args )
{
   //Initialize any member variables that need to be used inside of LoadMap() here.
   //Note: At this point, the Managers are not yet initialized. The Engine initialization
   //occurs immediately after this method returns (see GLViewAssignment_1::New() for
   ...
   //GLViewAssignment_1::onCreate() is invoked after this module's LoadMap() is completed.
}


void GLViewAssignment_1::onCreate()
{
   //GLViewAssignment_1::onCreate() is invoked after this module's LoadMap() is completed.
...
GLViewAssignment_1::~GLViewAssignment_1()
{
   //Implicitly calls GLView::~GLView()
}


void GLViewAssignment_1::updateWorld()
{
...
void GLViewAssignment_1::onResizeWindow( GLsizei width, GLsizei height )
{
   GLView::onResizeWindow( width, height ); //call parent's resize method.
}


void GLViewAssignment_1::onMouseDown( const SDL_MouseButtonEvent& e )
{
   GLView::onMouseDown( e );
}


void GLViewAssignment_1::onMouseUp( const SDL_MouseButtonEvent& e )
{
   GLView::onMouseUp( e );
}


void GLViewAssignment_1::onMouseMove( const SDL_MouseMotionEvent& e )
{
   GLView::onMouseMove( e );
}


void GLViewAssignment_1::onKeyDown( const SDL_KeyboardEvent& key )
{
...
void GLViewAssignment_1::onKeyUp( const SDL_KeyboardEvent& key )
{
   GLView::onKeyUp( key );
}


void Aftr::GLViewAssignment_1::loadMap()
{
  ...
  // skyBoxImageNames.push_back( ManagerEnvironmentConfiguration::getSMM() + "/images/skyboxes/sky_afternoon+6.jpg" );
...

   {
      //Create the SkyBox
       ...
       //Tried to create a day/night cycle but couldn't figure out how. The commented out code is my attempt at it.

       //static int sky_box = 0; //Type of sky

       //static std::chrono::seconds begin(20);
       //static std::chrono::seconds stop(0);
     ...
     /* std::chrono::duration<double> dur = end - start;
      ...
      std::cout << "current time: " << dur << "secs\n";

      if ((start.time_since_epoch() % begin) == stop ) {
   ...
   createAssignment_1WayPoints();
}


void GLViewAssignment_1::createAssignment_1WayPoints()
{
